...
def model() -> Model:
    go_model = Sequential()
    go_model.add(Conv2D(32, kernel_size=(4, 4),activation='linear',input_shape=(8,8,1),padding='same'))
    go_model.add(LeakyReLU(alpha=0.1))
    go_model.add(MaxPooling2D((2, 2),padding='same'))
    go_model.add(Conv2D(64, (4, 4), activation='linear',padding='same'))
    go_model.add(LeakyReLU(alpha=0.1))
    go_model.add(MaxPooling2D(pool_size=(2, 2),padding='same'))
    go_model.add(Conv2D(128, (4, 4), activation='linear',padding='same'))
    go_model.add(LeakyReLU(alpha=0.1))                  
    go_model.add(MaxPooling2D(pool_size=(2, 2),padding='same'))
