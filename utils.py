...
def load_train_data(model_gen: str) -> \
    tuple[tuple[np.ndarray, np.ndarray], tuple[np.ndarray, np.ndarray]]:
    
    # Calculate the number of trial epochs to load.
    num_test_trials = len(next(os.walk(f'data/{model_gen}/'))[1]) - 2
    
    board_data = []
    move_data = []

    # Load training data from each epoch.
    for i in range(num_test_trials):
        board_files = [name for name in os.listdir(f'./data/{model_gen}/epoch_{i+1}/') if "board" in name]
        move_files = [name for name in os.listdir(f'./data/{model_gen}/epoch_{i+1}/') if "selected_move" in name]

        # Load Board Training Data
        for board_file in board_files:
            with open(f'./data/{model_gen}/epoch_{i+1}/{board_file}') as f:
        ...
        for move_file in move_files:     
            with open(f'./data/{model_gen}/epoch_{i+1}/{move_file}') as f: 
...
def prep_training_data(model_name: str ="test_training_set") -> \
    ...
    (train_X, train_Y), (test_X, test_Y) = load_train_data(model_name)
...
def load_model(model_name: str = "model_gen_0") -> Model:
